        ...
        <>
          <div className="agenda-page-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: '16px' }}>
            ...
            <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
              ...
              {onOpenTV && (
                <button className="btn-secondary" style={{ background: 'var(--text-primary)', color: 'var(--bg-primary)', border: '1px solid var(--text-primary)' }} onClick={onOpenTV}>
          ...
          <div className="agenda-filters-card">
            <div 
              ...
              style={{ 
                cursor: 'pointer', 
                userSelect: 'none',
                borderBottom: showFilters ? '1px solid var(--bg-glass)' : 'none',
            ...
            >
              <div className="agenda-filter-title" style={{ flex: '1 1 auto', minWidth: '250px', flexWrap: 'wrap' }}>
                ...
                <div>
                  <h3 style={{ margin: 0, fontSize: '1.1rem', fontWeight: 700, color: 'var(--text-primary)', display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
                    Filtros e Ações
                    <ChevronRight size={18} style={{ color: 'var(--text-secondary)', transition: 'transform 0.3s', transform: showFilters ? 'rotate(90deg)' : 'rotate(0deg)' }} />
                  </h3>
                  <p style={{ margin: 0, fontSize: '0.85rem', color: 'var(--text-secondary)' }}>Refine a agenda por equipe, período e status</p>
                </div>
              </div>
              ...
              <div style={{ flex: 1, width: '100%' }}>
                ...
                <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap', alignItems: 'flex-end' }}>
                  ...
                  <div className="agenda-input-group" style={{ display: 'flex', alignItems: 'flex-end' }}>
                    <div className="agenda-input-wrapper" style={{ height: '36px', padding: '0 12px', cursor: 'pointer', background: noDateOnly ? 'var(--bg-primary)' : 'var(--bg-secondary)' }} onClick={() => setNoDateOnly(!noDateOnly)}>
                      <label style={{ display: 'flex', alignItems: 'center', gap: '10px', cursor: 'pointer', fontSize: '0.85rem', fontWeight: 600, color: 'var(--text-secondary)', width: '100%', height: '100%' }} onClick={e => e.stopPropagation()}>
          ...
          <div className="agenda-surgeries-container">
            {Object.keys(groupedSurgeries).length === 0 && !loading && (
              <div style={{ textAlign: 'center', padding: '40px', color: 'var(--text-secondary)' }}>Nenhuma cirurgia encontrada com os filtros atuais.</div>
            )}
                      ...
                      return (
                        <div key={surgery.id} className="agenda-card" onClick={() => onViewClick && onViewClick(surgery)} style={{ cursor: 'pointer' }}>
                          ...
                          <div className="agenda-info-grid" style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '20px' }}>
                            ...
                            <div style={{ display: 'flex', gap: '16px', marginTop: '4px', paddingTop: '8px', borderTop: '1px dashed var(--border-glass)' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', marginTop: '40px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '16px', background: '#fff', padding: '8px 16px', borderRadius: '20px', border: '1px solid var(--border-glass)', boxShadow: '0 2px 10px rgba(0,0,0,0.02)' }}>
              <button 
                ...
                style={{ padding: '6px', borderRadius: '50%', border: 'none', background: currentPage === 1 ? 'transparent' : 'var(--bg-glass)', color: currentPage === 1 ? 'var(--border-glass)' : 'var(--text-secondary)', cursor: currentPage === 1 ? 'default' : 'pointer' }}
              >
                <ChevronLeft size={18} />
              </button>
              ...
              <button 
                ...
                style={{ padding: '6px', borderRadius: '50%', border: 'none', background: currentPage >= Math.ceil(totalCount / pageSize) || totalCount === 0 ? 'transparent' : 'var(--bg-glass)', color: currentPage >= Math.ceil(totalCount / pageSize) || totalCount === 0 ? 'var(--border-glass)' : 'var(--text-secondary)', cursor: currentPage >= Math.ceil(totalCount / pageSize) || totalCount === 0 ? 'default' : 'pointer' }}
              >
      ...
      {viewMode === 'full' && (
        ...
        <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          <button 
            ...
            style={{ width: 'auto', display: 'flex', alignItems: 'center', gap: '8px', padding: '10px 20px', borderRadius: '10px', fontWeight: 600, border: '1px solid var(--border-glass)', transition: 'all 0.2s', background: 'var(--bg-primary, var(--bg-secondary))', color: 'var(--text-primary)' }}
          >
            <LayoutGrid size={18} />
            Modo Agenda
          </button>
                ...
                <tbody>
                  {surgeries.map((surgery) => (
                    ...
                    >
                      ...
                      <td><div style={{ fontSize: '0.85rem', fontWeight: '500', color: (surgery.carater === 'URGÊNCIA' || surgery.carater === 'URGENCIA') ? '#f87171' : (surgery.carater === 'JUDICIAL' ? '#c084fc' : 'var(--text-secondary)') }}>{surgery.carater || '-'}</div></td>
                      <td>
                        <div 
                    ...
                    <tbody>
                      {surgeries.map((surgery) => (
                        ...
                        >
                          ...
                          <td><div style={{ padding: '0 8px', fontSize: '0.8rem', whiteSpace: 'nowrap', fontWeight: '500', color: (surgery.carater === 'URGÊNCIA' || surgery.carater === 'URGENCIA') ? '#f87171' : (surgery.carater === 'JUDICIAL' ? '#c084fc' : 'var(--text-secondary)') }}>{surgery.carater || '-'}</div></td>
