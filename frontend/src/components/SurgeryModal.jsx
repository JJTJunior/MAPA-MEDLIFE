...
class SurgeryModalErrorBoundary extends React.Component {
  constructor(props) {
    super(props);
  ...
  componentDidCatch(error, errorInfo) {
    console.error("SurgeryModal Error Boundary:", error, errorInfo);
  }
  render() {
        ...
        <div className="modal-overlay" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 999999 }}>
          <div style={{ background: '#ffffff', color: '#991b1b', padding: '30px', borderRadius: '12px', maxWidth: '600px', width: '90%', boxShadow: '0 20px 25px -5px rgba(0,0,0,0.5)', border: '2px solid #ef4444' }}>
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
              <button onClick={() => window.location.reload()} style={{ background: '#2563eb', color: '#fff', border: 'none', padding: '10px 20px', borderRadius: '6px', fontWeight: 600, cursor: 'pointer' }}>
                Recarregar Página
              </button>
            </div>
          </div>
...
function SurgeryModalInner({ isOpen, onClose, surgery, user, onSaveSuccess }) {
  ...
  const [caraterList, setCaraterList] = useState([
    { name: 'ELETIVA' },
    { name: 'URGÊNCIA' },
    { name: 'JUDICIAL' }
  ]);
  ...
  const fetchLists = async () => {
    try {
      ...
      try {
        ...
        if (caraterData && caraterData.length > 0) {
          setCaraterList(caraterData);
        } else {
          const stored = localStorage.getItem('carater_list');
          if (stored) setCaraterList(JSON.parse(stored));
        }
      } catch (e) {
        const stored = localStorage.getItem('carater_list');
        if (stored) setCaraterList(JSON.parse(stored));
  ...
  useEffect(() => {
    ...
    carater: '',
        ...
        instrumentalist1: '',
        instrumentalist2: '',
        salesperson: (user?.role === 'Vendedor' && user?.email) ? user.email.split('@')[0].toUpperCase() : '',
        surgery_type: '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      onSaveSuccess(surgery ? { ...surgery, ...payload } : null);
      onClose();
    } catch (err) {
          ...
          <div className="form-grid">
            ...
            <div className="form-group">
              ...
              >
                ...
                {caraterList.map((c, idx) => (
                  <option key={idx} value={c.name}>{c.name}</option>
                ))}
                {formData.carater && !caraterList.find(c => c.name === formData.carater) && (
                  <option value={formData.carater}>{formData.carater} (Legado)</option>
                )}
              </select>
            </div>
            ...
            <div className="form-group form-grid-full">
              ...
              <div 
                ...
                onFocus={() => setIsDropzoneFocused1(true)}
...
export default function SurgeryModal(props) {
  ...
  return (
    <SurgeryModalErrorBoundary>
      <SurgeryModalInner {...props} />
    </SurgeryModalErrorBoundary>
  );
}
