...
import { Calendar, CheckCircle2, AlertTriangle, Clock, ShieldAlert, Award, Settings, FileText, TrendingUp, Activity, History, AlertCircle, PackageSearch, PackageCheck, Map, Eye } from 'lucide-react';
...
export default function Dashboard({ user, onNavigate, onlineUsers, onOpenOnlineModal }) {
  ...
  const getStartDate = () => {
    ...
    else if (periodFilter === 'year') d.setFullYear(d.getFullYear() - 1);
    else return null;
  ...
  const getEndDate = () => {
    if (periodFilter === 'last_month') {
      const d = new Date();
      d.setDate(0); // Último dia do mês passado
      return d.toISOString().split('T')[0];
    ...
    try {
      ...
      if (format === 'excel') {
        const exportDataRows = data.map(item => ({
          ...
          'Anexo': ((item.medical_request_urls && item.medical_request_urls.length > 0) || item.attachment_url) ? 'Sim' : 'Não',
  ...
  const fetchStats = async () => {
    ...
    try {
      ...
      let qCharts = supabase.from('surgeries').select('hospital, doctor, salesperson, insurance, date, status, surgery_type').order('date', { ascending: false }).limit(2000);
  ...
  return (
    ...
    <div className="dashboard-container">
      ...
      <div className="kpi-grid">
        ...
        {/* KPI 4 */}
        <div className="kpi-card glass-card" style={{ cursor: 'pointer', transition: 'transform 0.2s' }} onClick={() => handleNavigate({ status: 'URGÊNCIA' })} onMouseEnter={(e) => e.currentTarget.style.transform = 'scale(1.02)'} onMouseLeave={(e) => e.currentTarget.style.transform = 'scale(1)'}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
            <span className="kpi-title">Urgências</span>
            <ShieldAlert size={20} style={{ color: '#60a5fa' }} />
          </div>
          <div className="kpi-value" style={{ color: '#60a5fa' }}>{stats.urgent}</div>
          <span className="kpi-footer">Cirurgias emergenciais ativas</span>
        </div>

        {/* KPI 5: Aguardando Autorização */}
