...
import { Search, ChevronLeft, ChevronRight, Edit2, Plus, Filter, Check, X, RefreshCw, Clock, Download, ArrowLeft, Printer, FileText, MessageCircle, List, LayoutGrid, Info, Eye, Stethoscope, Building2, CreditCard, User, Smartphone, Calendar, Upload, Activity, Paperclip, Share2 } from 'lucide-react';
...
export default function SurgeryGrid({ user, initialFilters, onEditClick, onViewClick, onCreateClick, onBack, onOpenTV }) {
  ...
  useEffect(() => {
    ...
    fetchSurgeries();
  }, [patientFilter, hospitalFilter, doctorFilter, salespersonFilter, surgeryTypeFilter, selectedStatus, startDateFilter, endDateFilter, instrumentalist1Filter, instrumentalist2Filter, insuranceFilter, user, viewMode, columnFilters, noDateOnly]);
              ...
              <table className="data-table" style={{ zoom: tableZoom }}>
                <thead>
                  <tr>
                    ...
                    <th style={{ textAlign: 'center', paddingLeft: '4px', paddingRight: '4px', width: '1%' }}>ANEXO</th>
                    <th>Observação</th>
                  ...
                  {surgeries.map((surgery) => (
                    ...
                    >
                      ...
                      <td>
                        <button 
                          className="btn-icon" 
                          title={isEditable ? "Editar Cirurgia" : "Visualizar Detalhes"}
                          onClick={(e) => {
                            e.stopPropagation();
                            onEditClick(surgery);
                          }}
            ...
            <table className="print-table" style={{ width: '100%', borderCollapse: 'collapse', fontSize: '9px', color: 'black' }}>
              <thead>
                <tr style={{ backgroundColor: '#f3f4f6' }}>
                  ...
                  <th style={{ border: '1px solid #ccc', padding: '4px' }}>ANEXO</th>
              ...
              <tbody>
                {printData.map(item => (
                  <tr key={item.id}>
                    ...
                    <td style={{ border: '1px solid #ccc', padding: '4px' }}>{((item.medical_request_urls && item.medical_request_urls.length > 0) || item.attachment_url) ? '✓' : ''}</td>
              ...
              {shareModalData.files.filter(f => f.type === 'application/pdf').length > 0 && (
                ...
                >
                  ...
                  <div style={{ textAlign: 'left' }}>
                    ...
                    <div style={{ fontSize: '0.75rem', opacity: 0.9, fontWeight: 'normal' }}>Enviar PDF - Solicitação Médica / Autorização / Comanda</div>
                  </div>
                </button>
              )}
