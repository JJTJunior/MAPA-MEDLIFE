...
BEGIN
    ...
    INSERT INTO auth.users (
      ...
      jsonb_build_object('name', new_name, 'group_name', group_name, 'email_verified', true),
      now(), now(),
      '', '', '', '',
      '', '', '', '',
      0, false, false
    ...
    INSERT INTO auth.identities (
      ...
      jsonb_build_object('sub', new_uid::text, 'email', new_email, 'email_verified', true, 'phone_verified', false),
      'email', now(), now(), now()
    );
...
async function main() {
  try {
    ...
    // Also let's fix ALL existing users that have NULL in confirmation_token!
    console.log('Fixing all existing users with NULL tokens...');
    await client.query(`
      UPDATE auth.users 
      SET 
