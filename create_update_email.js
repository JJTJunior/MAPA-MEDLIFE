...
BEGIN
    ...
    -- 4. Atualizar email na identidade principal do usuário
    UPDATE auth.identities
    SET email = new_email,
        identity_data = jsonb_set(identity_data, '{email}', to_jsonb(new_email))
    WHERE user_id = target_user_id AND provider = 'email';

    -- 5. Atualizar perfil (caso a coluna email exista lá, se existir, não é padrão, mas vamos tentar atualizar se a coluna email existir ou o nome caso queira)
    -- Em user_profiles a gente tem o email?
    -- No schema padrão que criamos, user_profiles tem a coluna email.
...
async function main() {
  try {
    ...
    console.log('Creating admin_update_user_email...');
